...
import optuna
import yaml
from dataset_utils import build_augmentation, get_reindeer_dicts, split_dataset
    ...
    def build_train_loader(cls, cfg):
        return build_detection_train_loader(
            ...
            mapper=DatasetMapper(
                cfg, is_train=True, augmentations=build_augmentation(cfg, is_train=True)
            ),
        )
...
def objective(trial, output_dir):
    ...
    logger.info(
        f"OPTUNA: Starting trial {trial.number} with parameters: LR={cfg.SOLVER.BASE_LR}, MAX_ITER={cfg.SOLVER.MAX_ITER}, IMS_PER_BATCH={cfg.SOLVER.IMS_PER_BATCH}"
    )
