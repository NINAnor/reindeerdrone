...
def visualize_preds_with_gt(
    image, preds, annotations, output_path, class_names=["Adult", "Calf"]
):
    ...
    def draw_rectangle(ax, box, cls, label_text, color, style):
        x1, y1, x2, y2 = box
        rect = patches.Rectangle((x1, y1), x2 - x1, y2 - y1, edgecolor=color, **style)
        ax.add_patch(rect)
...
def filter_overlapping_boxes(results, iou_threshold=0.8):
    ...
    for i, (box1, score1, cls1, x_offset1, y_offset1) in enumerate(results):
        keep = True
        for j, (box2, score2, cls2, x_offset2, y_offset2) in enumerate(
            filtered_results
        ):
...
def load_predictor(config_file, model_weights, num_classes=2, threshold=0.5):
    ...
    cfg.MODEL.ROI_HEADS.NUM_CLASSES = (
        num_classes  # Update to match the number of classes
    )
...
def process_folder(cfg):
    ...
    for image_filename in tqdm(image_filenames, desc="Processing Images"):
        ...
        output_image_path = os.path.join(
            pred_output_folder,
            "image",
            f"{os.path.splitext(image_filename)[0]}_pred.png",
        )
        output_json_path = os.path.join(
            pred_output_folder,
            "json",
            f"{os.path.splitext(image_filename)[0]}_pred.json",
        )
